...
export function calculateMessageExpiry(
  ...
  if (lobby?.date && lobby?.endTime) {
    try {
      // Parse date (YYYY-MM-DD) and endTime (e.g., "20:00" or "8:00 PM")
      let endHour = 22;
      let endMinute = 0;
      ...
      // If valid and in the future (or within past 24h), use it
  ...
  // Fallback: 24 hours from right now
...
export async function sendLobbyChatMessage(params: {
  ...
  // Send push notification to other squad participants
  if (params.lobby?.players && params.type !== 'SYSTEM') {
    try {
      const otherRecipients = params.lobby.players
        .map((p) => p.playerId || p.uid)
        .filter((uid) => uid && uid !== params.senderId);

      for (const recipientId of otherRecipients) {
        sendPushNotification({
          recipientId,
          title: `Squad Chat: ${params.lobby.name}`,
          message: `${params.senderName}: ${cleanText.length > 50 ? cleanText.slice(0, 50) + '...' : cleanText}`,
          type: 'LOBBY' as any,
          relatedId: params.lobbyId,
          relatedType: 'LOBBY',
          data: {
            type: 'SQUAD_CHAT',
            lobbyId: params.lobbyId,
      ...
      console.warn('Squad chat push notification error:', notifErr);
    }
  }
...
export function subscribeLobbyMessages(
  ...
  const unsubscribe = onSnapshot(
    ...
    (snapshot) => {
      ...
      // Free background cleanup if expired messages are detected
 ...
 * 100% FREE Firestore batch delete.
 */
export async function cleanupExpiredLobbyMessages(lobbyId: string): Promise<number> {
  try {
    ...
    await batch.commit();
    console.log(`[TurFit Ephemeral Chat] Cleaned up ${snap.size} expired messages for lobby ${lobbyId}`);
    return snap.size;
  } catch (err) {
...
export async function purgeAllLobbyMessages(lobbyId: string): Promise<number> {
  try {
    ...
    await batch.commit();
    console.log(`[TurFit Ephemeral Chat] Purged all ${snap.size} messages for lobby ${lobbyId}`);
    return snap.size;
  } catch (err) {
